...
public class PriceController {
	...
	public List<TStock>refresh(){
		...
		for(TStock ts:list) {
			try {
				...
				//更新報價資訊
				tStockRepository.updatePrice(ts.getId(),
											 ts.getChangePrice(),
	...
	public List<HistoricalQuote>queryHistoricalQuotes(@PathVariable("symbol")String symbol){
		List<HistoricalQuote> queryHistQuotes=null;
		try {
			Calendar from = Calendar.getInstance();
			Calendar to = Calendar.getInstance();
			from.add(Calendar.YEAR, -1); // from 1 year ago
			Stock stock=YahooFinance.get("symbol");
			queryHistQuotes=stock.getHistory(from,to,Interval.DAILY);
